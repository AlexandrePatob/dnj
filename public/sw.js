 ...
 * revision: 9b99d2295472
 */
var revision = "9b99d2295472";
(() => {
  ...
  var LOCAL_HOSTNAMES = /* @__PURE__ */ new Set(["localhost", "::1"]);
  ...
  function classifyRequest(request, appOrigin) {
    ...
    if (isLocalDevelopmentOrigin(appOrigin) || request.method !== "GET" || url.origin !== appOrigin || request.headers.has("Authorization") || url.pathname === "/v1" || url.pathname.startsWith("/v1/") || url.pathname === "/api/v2" || url.pathname.startsWith("/api/v2/")) {
      return "network-only";
    }
  ...
  function createServiceWorkerRuntime(environment, revision) {
    ...
    async function activate() {
      ...
      await Promise.all(
        existing.filter((name) => name.startsWith(CACHE_PREFIX) && (development || !currentCaches.has(name))).map((name) => environment.caches.delete(name))
      );
  ...
  var pushRoutes = /* @__PURE__ */ new Set(["/?screen=queue", "/?screen=game", "/?screen=gallery", "/?screen=home"]);
  var isBoundedText = (value, max) => typeof value === "string" && value.trim().length > 0 && value.length <= max;
  function parsePushPayload(value) {
    if (!value || typeof value !== "object") return null;
    const payload = value;
    if (!isBoundedText(payload.notificationId, 128) || !isBoundedText(payload.title, 120) || !isBoundedText(payload.body, 500) || !isBoundedText(payload.tag, 160) || !pushRoutes.has(payload.url)) return null;
    const vibrate = Array.isArray(payload.vibrate) && payload.vibrate.length <= 10 && payload.vibrate.every((item) => Number.isInteger(item) && item >= 0 && item <= 1e4) ? payload.vibrate : void 0;
    return { notificationId: payload.notificationId, title: payload.title, body: payload.body, url: payload.url, tag: payload.tag, ...vibrate ? { vibrate } : {} };
  }
  async function openPushTarget(clients, url) {
  ...
  if (typeof scope.addEventListener === "function" && typeof scope.skipWaiting === "function" && scope.caches) {
    const revision = true ? "9b99d2295472" : "development";
    ...
    scope.addEventListener("notificationclick", ((event) => {
      ...
      if (pushRoutes.has(url)) event.waitUntil(openPushTarget(scope.clients, url));
    }));
  }
})();
