...
beforeEach(() => {
  ...
  fetchMock.mockImplementation((input: string) => {
    if (input === "/api/admin/moderation?queue=challenge") return Promise.resolve(new Response(JSON.stringify({ moments: [{ id: "moment-1", captured_at: "2026-10-18T17:35:00.000Z", points_awarded: 30, moderation_status: "approved", reward_status: "awarded", photo_status: "available", imageUrl: "/api/v1/media/private/moment-1.jpg", participation: { participantName: "Alex", experienceName: "Foto com a galera", isChallenge: true } }] })));
...
describe("AdminDashboard moderation", () => {
  it("shows the challenge photo and only corrective decisions", async () => {
    ...
    expect(screen.getByRole("img", { name: "Foto de Alex" })).toHaveAttribute("src", "/api/v1/media/private/moment-1.jpg");
    expect(screen.getByRole("link", { name: "Abrir foto de Alex em tamanho maior" })).toHaveAttribute("href", "/api/v1/media/private/moment-1.jpg");
