...
function ActivityList() {
  const [activities, setActivities] = useState<Activity[] | null>(null); const [spaces, setSpaces] = useState<Space[]>([]); const [error, setError] = useState(""); const [message, setMessage] = useState(""); const [editing, setEditing] = useState<Activity | null>(null); const [formOpen, setFormOpen] = useState(false); const [name, setName] = useState(""); const [slug, setSlug] = useState(""); const [description, setDescription] = useState(""); const [kind, setKind] = useState("challenge"); const [spaceId, setSpaceId] = useState(""); const [checkInPoints, setCheckInPoints] = useState("10"); const [momentPoints, setMomentPoints] = useState("20"); const [cooldownSeconds, setCooldownSeconds] = useState("60"); const [allowsMoment, setAllowsMoment] = useState(true); const [startsAt, setStartsAt] = useState(""); const [endsAt, setEndsAt] = useState("");
  const load = useCallback(async () => { try { const data = await api<{ data: Activity[] }>("/admin/activities"); setActivities(data.data); } catch { setError("Não foi possível carregar as atividades."); } }, []);
  ...
  function closeForm() { reset(); setFormOpen(false); }
  function openNew() { setError(""); setMessage(""); reset(); setFormOpen(true); }
  function edit(activity: Activity) { setError(""); setMessage(""); setEditing(activity); setName(activity.name); setSlug(activity.slug); setDescription(activity.description ?? ""); setKind(activity.kind); setSpaceId(activity.spaceId ?? ""); setCheckInPoints(String(activity.checkInPoints)); setMomentPoints(String(activity.momentPoints)); setCooldownSeconds(String(activity.cooldownSeconds)); setAllowsMoment(activity.allowsMoment); setStartsAt(toDeviceDateTimeInput(activity.startsAt)); setEndsAt(toDeviceDateTimeInput(activity.endsAt)); setFormOpen(true); }
  async function save(event: FormEvent) { event.preventDefault(); const start = startsAt ? deviceDateTimeToUtc(startsAt) : null; const end = endsAt ? deviceDateTimeToUtc(endsAt) : null; if ((startsAt && !start) || (endsAt && !end) || (start && end && new Date(end) <= new Date(start))) { setError("O fim deve ser posterior ao início."); return; } try { const action = editing ? "alterações salvas" : "atividade criada"; await api(editing ? `/admin/activities/${editing.id}` : "/admin/activities", { method: editing ? "PATCH" : "POST", body: { name, slug: slugify(slug), description: description || null, kind, spaceId: spaceId || null, checkInPoints: Number(checkInPoints), momentPoints: Number(momentPoints), cooldownSeconds: Number(cooldownSeconds), allowsMoment, startsAt: start, endsAt: end } }); await load(); closeForm(); setMessage(`Atividade: ${action}.`); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível salvar a atividade."); } }
  async function updateStatus(activity: Activity, status: "active" | "paused") { try { setError(""); setMessage(""); await api(`/admin/activities/${activity.id}`, { method: "PATCH", body: { status } }); setActivities((current) => current?.map((item) => item.id === activity.id ? { ...item, status } : item) ?? null); setMessage(`Atividade ${status === "active" ? "ativada" : "pausada"}.`); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível atualizar o status da atividade."); } }
  async function remove(activity: Activity) { if (!window.confirm(`Arquivar a atividade “${activity.name}”? Ela deixará de aparecer neste catálogo.`)) return; try { setError(""); setMessage(""); await api(`/admin/activities/${activity.id}`, { method: "PATCH", body: { status: "archived" } }); setActivities((current) => current?.filter((item) => item.id !== activity.id) ?? null); if (editing?.id === activity.id) closeForm(); setMessage("Atividade arquivada."); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível arquivar a atividade."); } }
  if (error && !activities) return <Failure message={error} />;
  return <div className={styles.dashboard}><section className={styles.activity}><SectionTitle kicker="Programação" title="Atividades cadastradas" /><div className={styles.activityToolbar}><p className={styles.help}>Crie e edite atividades sem sair da listagem.</p><button className={styles.primaryButton} onClick={openNew}>Nova atividade</button></div>{error && <p role="alert" className={styles.operationError}>{error}</p>}{message && <p role="status" className={styles.operationMessage}>{message}</p>}{!activities ? <Loading /> : activities.length ? <ol>{activities.map((activity) => <li key={activity.id}><span className={styles.activityDot} /><div><strong>{activity.name}</strong><p>{activity.kind} · {activity.status} · {activity.checkInPoints} ponto(s) no check-in{activity.startsAt ? ` · inicia ${formatDate(activity.startsAt)}` : " · sem início definido"}{activity.endsAt ? ` · encerra ${formatDate(activity.endsAt)}` : ""}</p></div><span className={styles.rowActions}><button className={styles.ghostButton} onClick={() => edit(activity)}>Editar</button>{activity.status === "draft" && <button className={styles.primaryButton} onClick={() => void updateStatus(activity, "active")}>Ativar</button>}{activity.status === "active" && <button className={styles.ghostButton} onClick={() => void updateStatus(activity, "paused")}>Pausar</button>}<button className={styles.dangerButton} onClick={() => void remove(activity)}>Excluir</button></span></li>)}</ol> : <Empty text="Nenhuma atividade cadastrada." />}</section>{formOpen && <div className={styles.dialogBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeForm(); }}><form className={`${styles.formCard} ${styles.activityDialog}`} role="dialog" aria-modal="true" aria-labelledby="activity-dialog-title" onSubmit={save} onKeyDown={(event) => { if (event.key === "Escape") closeForm(); }}><SectionTitle kicker="Programação" title={editing ? `Editar: ${editing.name}` : "Nova atividade"} /><h2 id="activity-dialog-title" className={styles.srOnly}>{editing ? `Editar ${editing.name}` : "Nova atividade"}</h2><p className={styles.help}>Início e fim são independentes. Deixe qualquer um vazio para uma atividade sem horário fixo ou sem encerramento definido.</p><div className={styles.formGrid}><label>Nome<input autoFocus value={name} onChange={(event) => { setName(event.target.value); if (!editing) setSlug(slugify(event.target.value)); }} required /></label><label>Slug<input value={slug} onChange={(event) => setSlug(slugify(event.target.value))} pattern="[a-z0-9]+(-[a-z0-9]+)*" required /></label><label>Descrição<textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={4000} rows={3} /></label><label>Espaço<select value={spaceId} onChange={(event) => setSpaceId(event.target.value)}><option value="">Sem espaço</option>{spaces.map((space) => <option key={space.id} value={space.id}>{space.name}</option>)}</select></label><label>Tipo<select value={kind} onChange={(event) => setKind(event.target.value)}><option value="schedule">Agenda</option><option value="checkpoint">Check-in</option><option value="challenge">Desafio</option><option value="competitive">Competitiva</option><option value="live">Ao vivo</option></select></label><label>Início (opcional)<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></label><label>Fim (opcional)<input type="datetime-local" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} /></label><label>Pontos no check-in<input type="number" min="0" value={checkInPoints} onChange={(event) => setCheckInPoints(event.target.value)} required /></label><label>Pontos por momento<input type="number" min="0" value={momentPoints} onChange={(event) => setMomentPoints(event.target.value)} required /></label><label>Intervalo entre check-ins (segundos)<input type="number" min="0" value={cooldownSeconds} onChange={(event) => setCooldownSeconds(event.target.value)} required /></label><label className={styles.help}><input type="checkbox" checked={allowsMoment} onChange={(event) => setAllowsMoment(event.target.checked)} /> Permitir envio de momento</label></div><div className={styles.formFooter}><button className={styles.primaryButton}>{editing ? "Salvar alterações" : "Criar atividade"}</button><button type="button" className={styles.ghostButton} onClick={closeForm}>Cancelar</button>{error && <p role="alert">{error}</p>}</div></form></div>}</div>;
}
