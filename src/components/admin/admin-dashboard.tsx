...
function ActivityList() {
  const [activities, setActivities] = useState<Activity[] | null>(null); const [error, setError] = useState(""); const [name, setName] = useState(""); const [slug, setSlug] = useState(""); const [kind, setKind] = useState("challenge"); const [startsAt, setStartsAt] = useState(toDateTimeLocal); const [duration, setDuration] = useState("5");
  ...
  useEffect(load, [load]);
  async function create(event: FormEvent) { event.preventDefault(); const start = new Date(startsAt); if (Number.isNaN(start.getTime())) { setError("Informe um início válido."); return; } try { await api("/admin/activities", { method: "POST", headers: { "Idempotency-Key": newIdempotencyKey() }, body: JSON.stringify({ name, slug, kind, checkInPoints: 0, momentPoints: 0, cooldownSeconds: 0, allowsMoment: kind === "challenge", startsAt: start.toISOString(), endsAt: new Date(start.getTime() + Number(duration) * 60_000).toISOString() }) }); setName(""); setSlug(""); setStartsAt(toDateTimeLocal()); load(); } catch { setError("Não foi possível criar a atividade."); } }
  if (error && !activities) return <Failure message={error} />;
  return <div className={styles.dashboard}><form className={styles.formCard} onSubmit={create}><SectionTitle kicker="Programação" title="Nova atividade" /><div className={styles.formGrid}><label>Nome<input value={name} onChange={(event) => setName(event.target.value)} required /></label><label>Slug<input value={slug} onChange={(event) => setSlug(event.target.value)} pattern="[a-z0-9]+(-[a-z0-9]+)*" required /></label><label>Tipo<select value={kind} onChange={(event) => setKind(event.target.value)}><option value="schedule">Agenda</option><option value="checkpoint">Check-in</option><option value="challenge">Desafio</option><option value="competitive">Competitiva</option><option value="live">Ao vivo</option></select></label><label>Início<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} required /></label><label>Duração<select value={duration} onChange={(event) => setDuration(event.target.value)}><option value="1">1 min</option><option value="5">5 min</option><option value="10">10 min</option><option value="15">15 min</option></select></label></div><div className={styles.formFooter}><button className={styles.primaryButton}>Criar atividade</button>{error && <p role="alert">{error}</p>}</div></form><section className={styles.activity}><SectionTitle kicker="Catálogo" title="Atividades cadastradas" />{!activities ? <Loading /> : activities.length ? <ol>{activities.map((activity) => <li key={activity.id}><span className={styles.activityDot} /><div><strong>{activity.name}</strong><p>{activity.kind} · {activity.status} · {activity.checkInPoints} ponto(s) no check-in</p></div></li>)}</ol> : <Empty text="Nenhuma atividade cadastrada." />}</section></div>;
}
