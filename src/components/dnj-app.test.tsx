...
describe("DnjApp session restoration", () => {
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/auth/session")) return { ok: true, headers, json: async () => ({ accessToken: "restored-token", user: { id: "user-1", email: "ana@example.com", name: "Ana Souza", mobilePhone: "", documentMasked: "***", role: "DEFAULT", group: { id: "group-1", name: "Grupo Esperanca" }, onboardingComplete: true, avatarUrl: "https://images.example/ana.jpg" }, onboardingRequired: false }) } as Response;
