  ...
  return (
    <div className={styles.stack}>
      <section className={styles.panel}>
        ...
        {now.length ? <div className={styles.scheduleCards}>{now.map((item) => <ScheduleCard key={item.id} item={item} nowMs={nowMs} busy={busyId === item.id} operate={operate} openManual={openManual} />)}</div> : <p className={styles.empty}><Clock3 size={28} /><strong>Nenhuma atividade acontecendo agora</strong><br /><span>As atividades em andamento aparecerão aqui agrupadas por espaço.</span></p>}
      </section>
