...
import { Award, QrCode, Trophy } from "lucide-react";
...
function RankingRow({ item, position, group, current }: { item: { name: string; points: number; group?: string; members?: number }; position: number; group?: boolean; current?: boolean }) {
  return <div className="flex items-center gap-3 px-4 py-3" style={{ background: current ? "var(--primary-alpha-15)" : position <= 3 ? "color-mix(in srgb, var(--accent) 10%, var(--card))" : "transparent", borderBottom: "1px solid var(--border)" }}>
    {group ? position === 1 ? <Trophy size={23} style={{ color: "var(--primary)" }} /> : <span className="flex w-6 justify-center text-sm font-black" style={{ color: "var(--muted-foreground)" }}>{position}</span> : <MedalBadge position={position} />}
