...
export function EventMapScreen({ onBack }: { animDir: AnimDir; onBack: () => void }) {
  ...
  return <main className="absolute inset-0 overflow-y-auto px-5 pb-28" style={{ background: "var(--background)", paddingTop: "calc(var(--participant-header-height) + var(--safe-area-top))" }}>
    ...
    <section className="mt-5 overflow-hidden rounded-2xl" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <button type="button" onClick={openExpanded} className="relative block w-full text-left" aria-label="Ampliar mapa oficial">
        <Image src="/images/mapa-isometrico-dnj.png" alt="Mapa isométrico oficial do evento DNJ 2026" width={2714} height={1445} sizes="(max-width: 640px) 100vw, 768px" className="aspect-[16/9] w-full object-contain" priority />
