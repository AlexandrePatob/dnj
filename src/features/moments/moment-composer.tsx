...
export function MomentComposer({ participation, onClose, onCreated }: { participation: Participation; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setStatus("Momento enviado para moderaÃ§Ã£o.");
      window.setTimeout(onCreated, 700);
    } catch (error) {
      setStatus((error as { message?: string }).message ?? "NÃ£o foi possÃ­vel enviar agora.");
    }
  }
  return <section className="absolute inset-0 z-50 overflow-y-auto px-5" style={{ background: "var(--background)", paddingTop: "calc(58px + var(--safe-area-top))" }} aria-label="Compartilhar momento">
    ...
    <h2 className="text-2xl font-black">Compartilhar momento</h2><p className="mt-1 text-sm" style={{ color: "var(--muted-foreground)" }}>{participation.place.name} Â· seus registros aparecem primeiro em Meus registros.</p>
    ...
    <button type="button" onClick={() => inputRef.current?.click()} className="mt-6 flex aspect-square w-full items-center justify-center overflow-hidden rounded-3xl" style={{ background: preview ? "transparent" : "var(--muted)", border: "1px dashed var(--primary)" }}>{preview ? <img src={preview} alt="PrÃ©via do momento" className="h-full w-full object-cover" /> : <span className="text-center"><Camera className="mx-auto mb-3" style={{ color: "var(--primary)" }} /><strong>Capturar ou escolher foto</strong></span>}</button>
    <label className="mt-5 flex gap-3 rounded-2xl p-4 text-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-1" /><span><strong>Autorizar publicaÃ§Ã£o na Galeria DNJ</strong><br /><small style={{ color: "var(--muted-foreground)" }}>Seu registro passa por moderaÃ§Ã£o antes de aparecer publicamente.</small></span></label>
