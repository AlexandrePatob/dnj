...
import type { Moment, Participation } from "@/types/experience";
...
export function MomentComposer({ participation, onClose, onCreated }: { participation: Participation | null; onClose: () => void; onCreated: (moment: Moment) => void }) {
  ...
  async function submit() {
    ...
    setStatus("hashing");
    try {
      const moment = await publishMoment({ file, participationId: participation?.id, publishConsent: true, onProgress: (value: PublishProgress) => setStatus(value) });
      setStatus(moment.pointsAwarded === undefined || moment.pointsAwarded > 0 ? "success" : "published_without_points");
