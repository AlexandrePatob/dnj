...
type QueueStage="checking"|"select"|"preparing"|"tracking"|"completed";
...
export function QueueScreen({animDir,user={id:"anonymous",name:"Participante"},onQueueNotification}:{animDir:AnimDir;user?:{id:string;name:string};onQueueNotification?:(notification: LiveQueueNotification | null)=>void}){
 ...
 async function enter(next:"confession"|"spiritual"){if(!pastoralFirestore){setError("Fila indisponível neste ambiente.");return}if(operation)return;setOperation("enter");try{setError("");await joinQueue(pastoralFirestore,user,next);joinedThisSessionRef.current=true;setType(next);setStage("tracking")}catch(e){setError(e instanceof Error?e.message:"Não foi possível entrar na fila.")}finally{setOperation(null)}}
 ...
 return <div className="absolute inset-0 overflow-y-auto px-5 pb-[calc(var(--bottom-nav-total-height)+1rem)]" style={{background:"var(--background)",paddingTop:"calc(var(--participant-header-height) + 22px + var(--safe-area-top))",animation:animDir==="left"?"slideInLeft 280ms both":"fadeUp 220ms both"}}>{error&&<p role="alert" className="mt-3 rounded-xl p-3 text-sm" style={{background:"var(--red-alpha-12)",color:"var(--secondary)"}}>{error}</p>}
