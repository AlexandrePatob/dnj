...
export function QrSuccessCelebration({ points, label, scored = false, durationMs = 900, onDone }: { points: number; label: string; scored?: boolean; durationMs?: number; onDone: () => void }) {
  ...
  return <motion.section className="absolute inset-0 z-[70] grid place-items-center overflow-hidden bg-[#0d1a1a]/95 p-6 text-center text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} aria-live="assertive" aria-label={scored ? "Pontos creditados" : "Participação confirmada"}>
