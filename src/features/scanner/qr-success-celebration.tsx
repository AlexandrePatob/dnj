...
export function QrSuccessCelebration({ points, label, scored = false, alreadyRegistered = false, durationMs = 900, onDone }: { points: number; label: string; scored?: boolean; alreadyRegistered?: boolean; durationMs?: number; onDone: () => void }) {
  ...
  return <motion.section className="absolute inset-0 z-[70] grid place-items-center overflow-hidden bg-[#0d1a1a]/95 p-6 text-center text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} aria-live="assertive" aria-label={announcement}>
    ...
    <motion.div initial={reducedMotion ? false : { scale: .66, y: 14 }} animate={{ scale: 1, y: 0 }} transition={{ type: "spring", stiffness: 300, damping: 19 }}><span className="mx-auto grid h-20 w-20 place-items-center rounded-[28px]" style={{ background: "var(--game)", color: "#0d1a1a" }}><CheckCircle2 size={42} strokeWidth={2.8} /></span><p className="mt-7 text-sm font-bold uppercase tracking-[.18em] text-white/70">{alreadyRegistered ? "Atividade já pontuada" : scored ? "Pontos creditados" : "Entrada na partida confirmada"}</p><h2 className={`mt-2 font-black ${alreadyRegistered ? "text-3xl leading-tight sm:text-4xl" : "text-2xl"}`}>{alreadyRegistered ? "Você já pontuou nessa atividade!" : `+${points} pontos`}</h2><p className="mt-2 text-sm text-white/75">{alreadyRegistered ? "Sua participação está garantida no DNJ Game." : label}</p>{alreadyRegistered && <p className="mt-1 text-xs text-white/55">{label}</p>}<Sparkles className="mx-auto mt-4" size={19} style={{ color: "var(--game)" }} /></motion.div>
  </motion.section>;
}
