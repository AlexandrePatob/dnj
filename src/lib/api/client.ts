...
const csrfToken = () => publishedCsrfToken() ?? currentCsrfToken;
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> { try { return await requestOnce<T>(path, options); } catch (error) { if (!(error instanceof ApiError) || error.status !== 401 || path === "/auth/refresh" || options.refreshOnUnauthorized === false) throw error; refreshPromise ??= requestOnce("/auth/refresh", { method: "POST", refreshOnUnauthorized: false }).then(() => true).catch(() => false).finally(() => { refreshPromise = null; }); if (!(await refreshPromise)) throw error; return requestOnce<T>(path, { ...options, headers: { ...(currentCsrfToken ? { "X-CSRF-Token": currentCsrfToken } : {}), ...options.headers } }); } }
