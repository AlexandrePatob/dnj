...
describe("groupsApi", () => {
  ...
  it("creates a group through the authenticated V2 endpoint", () => { groupsApi.create({ name: "Jovens da Luz" }, "session-token"); expect(apiMutation).toHaveBeenCalledWith("/groups", expect.objectContaining({ method: "POST", token: "session-token", body: { name: "Jovens da Luz" } })); });
