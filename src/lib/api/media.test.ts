...
describe("publishMoment", () => {
  ...
  it("orders checksum, intent, signed PUT, bodyless complete and publish with exact payloads", async () => {
    const order: string[] = [];
    vi.mocked(apiMutation).mockImplementation(async (path, options) => { order.push(path); if (path === "/media/upload-intents") { expect(options.body).toEqual({ contentType: "image/jpeg", bytes: 3, checksumSha256: expect.stringMatching(/^[A-Za-z0-9+/]{43}=$/) }); expect(options.idempotencyKey).toBe("intent-key"); return { id: "asset-1", uploadUrl: "https://upload.test", method: "PUT", headers: { "Content-Type": "image/jpeg", "X-Amz-Checksum-Sha256": "signed-checksum" }, expiresAt: "2030-01-01T00:00:00Z" }; } if (path.endsWith("/complete")) { expect(options.body).toBeUndefined(); expect(options.idempotencyKey).toBe("complete-key"); return { id: "asset-1" }; } expect(path).toBe("/moments"); expect(options.body).toEqual({ mediaAssetId: "asset-1", participationId: "p1", publishConsent: true }); return { id: "moment-1" }; });
