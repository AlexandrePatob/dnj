...
export async function publishMoment(input: { file: File; participationId?: string; publishConsent: boolean; onProgress?: (state: PublishProgress) => void }): Promise<Moment> {
  ...
  const intent = await apiMutation<Intent>("/media/upload-intents", { method: "POST", body: { fileName: input.file.name, contentType: input.file.type, size: input.file.size, checksum: sha256 }, idempotencyKey: newIdempotencyKey() });
