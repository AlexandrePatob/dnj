...
describe("momentsApi", () => { it("passes one supported scope and opaque cursor", () => { momentsApi.list("group", "a/b?c"); expect(apiRequest).toHaveBeenCalledWith("/moments?scope=group&cursor=a%2Fb%3Fc"); }); it("uses the plural idempotent like endpoint", () => { momentsApi.like("m1", "key"); expect(apiMutation).toHaveBeenCalledWith("/moments/m1/likes", expect.objectContaining({ method: "POST", idempotencyKey: "key" })); }); });
